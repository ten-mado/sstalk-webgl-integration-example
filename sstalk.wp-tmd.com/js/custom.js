...
// SSTalkの起動
$('.sstalk_sticky_icon').on('click', function () {
	console.log('SSTalk モーダルの起動');
	$('.sstalk_modal').toggle();
	// 600pxか、画面の縦幅の90％
	...
	let width = height / 16 * 9; // 9:16縦画面
	$('.sstalk_modal').css('height', height);
	$('.sstalk_modal').css('width',width);
	// $('.sstalk_modal_iframe').css('','calc(50% - ' + height/2 + 'px)');
	// $('.sstalk_modal_iframe').css('left','calc(50% - ' + width/2 + 'px)');
	$('.sstalk_modal').css('right','15px');
	$('.sstalk_modal').css('bottom','15px');

	// クエリパラメータからuserとkeyを抽出
